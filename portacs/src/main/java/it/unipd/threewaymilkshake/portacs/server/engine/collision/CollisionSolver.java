...
public class CollisionSolver
    implements Handler<Map<SimplePoint, List<String>>, Map<String, Action>> {
    ...
    for (SimplePoint key : collisions.keySet()) {
      for (int i = 0; i < collisions.get(key).size() - 1; i++) {
        for (int j = i + 1; j < collisions.get(key).size(); j++) {
          // System.out.println(collisions.get(key).get(i).id + " " +
          // collisions.get(key).get(j).id);
  ...
  public void setNearest() {
    for (SimplePoint key : collisions.keySet()) {
      ...
      for (String unit : collisions.get(key)) {
        ...
        if (distance == 0
            || !response.get(unit).isInStop() && !response.get(unit).isCalculatingAgaing()) {
      ...
      if (equals.size() > 1) {
        ...
        for (int i = 0; i < equals.size(); i++) {
          if (i != random) response.get(equals.get(i)).add("STOP");
        }
      }
    }
  }
